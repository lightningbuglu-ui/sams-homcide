...
SWEP.Instructions = "This is a single cylindrical, flexible strand of metal connected to two ergonomic grips made of carbon fibre and metal. Use it to strangle people.\n\nLMB to swing.\nWhen strangling, press LMB to stop strangling."
...
-- FIX #4: Removed IsFromBehind — it was never called and always returned true (dead code)
...
function SWEP:PrimaryAttack()
    ...
    -- FIX #2: Added IsValid() guard before calling :IsPlayer() to prevent crash on invalid owner
    if self:GetLastAttack() ~= oldAttack and IsValid(self:GetOwner()) and self:GetOwner():IsPlayer() then
        local mul = 1
...
function SWEP:CustomThink()
    ...
    -- FIX #5: Removed duplicate `local ragPly` that was declared above but never used
...
if SERVER then
    ...
    hook.Add("HG_MovementCalc_2", "FiberwireSlowMove", function(mul, ply, cmd)
        ...
        -- FIX #1: Was "weapon_zc_fiberwire_standalone" — corrected to match the actual file/class name
        if wep:GetClass() ~= "weapon_fiberwire" then return end
...
if SERVER then
    ...
    -- FIX #3: "Should Fake Up" had a space in the hook name — it would never fire.
    -- Corrected to "ShouldFakeUp" to match GMod PascalCase hook naming.
    hook.Add("ShouldFakeUp", "FiberwireStrangleLockUp", function(ply)
        local r = ply and ply.FakeRagdoll
        if IsValid(r) and r.StrangleLocked then
            return true
        end
