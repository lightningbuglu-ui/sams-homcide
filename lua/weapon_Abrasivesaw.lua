...
SWEP.Weight = 0
SWEP.bloodID = 3
-- NOTE: both SWEP.Weight (0) and SWEP.weight (4.5) exist as separate fields due to
-- casing. Check which one your base class actually reads for weapon balancing/switch
-- priority, and remove whichever is unused to avoid confusion.
SWEP.weight = 4.5

SWEP.WorldModel = "models/weapons/tfa_nmrih/w_me_abrasivesaw.mdl"
SWEP.WorldModelReal = "models/weapons/tfa_nmrih/v_me_abrasivesaw.mdl"
SWEP.WorldModelExchange = false
SWEP.ViewModel = ""

SWEP.HoldType = "camera"
SWEP.HoldPos = Vector(-8, -2, 1)
...
-- Ammo types in this codebase are looked up by their display `name` field
-- (see hg.ammotypes in sh_ammostuff.lua), NOT by the table key "gascan".
-- This must match hg.ammotypes.gascan.name exactly or GetAmmoCount/RemoveAmmo
-- will silently look up a nonexistent ammo type and always report 0.
SWEP.GasCanAmmoType = "Gas Can"
SWEP.RefuelDuration = 2 -- seconds the player must hold Reload to refuel
SWEP.MaxGasoline = 100

-- Shared "Refueling..." HUD, used by any weapon that sets self.isRefueling /
-- self.refuelStartTime while the player holds Reload. Installed once no
-- matter how many weapon files load this block.
if CLIENT and not GasCanRefuelHUD_Installed then
    GasCanRefuelHUD_Installed = true

    hook.Add("HUDPaint", "GasCanRefuelHUD", function()
        local ply = LocalPlayer()
        if not IsValid(ply) then return end

        local wep = ply:GetActiveWeapon()
        if not IsValid(wep) or not wep.isRefueling or not wep.refuelStartTime then return end

        local duration = wep.RefuelDuration or 2
        local frac = math.Clamp((CurTime() - wep.refuelStartTime) / duration, 0, 1)

        local scrW, scrH = ScrW(), ScrH()
        local barW, barH = 260, 20
        local x = scrW * 0.5 - barW * 0.5
        local y = scrH * 0.5 + 90

        draw.RoundedBox(4, x - 10, y - 26, barW + 20, barH + 40, Color(0, 0, 0, 160))
        draw.SimpleText("REFUELING", "DermaDefaultBold", scrW * 0.5, y - 14, Color(255, 220, 150, 255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)

        draw.RoundedBox(3, x, y, barW, barH, Color(40, 40, 40, 220))
        draw.RoundedBox(3, x, y, barW * frac, barH, Color(220, 150, 40, 230))
    end)
end
...
-- FIX: This previously ran only in `if SERVER then`, which meant the owning
-- client never locally invoked PlayAnim("turnon"/"turnoff", ...) for the
-- viewmodel. Melee/viewmodel animations need to run in the predicted context
-- (server + owning client) via IsFirstTimePredicted(), same as StartHoldAttack.
-- NWVar changes and one-shot sound emission are still gated to SERVER so they
-- only fire once instead of once per predicted pass.
function SWEP:Reload()
    ...
    local isOn = self:GetNWBool("chainsawon", false)
    local isEmpty = self:GetNWInt("Gasoline", 0) <= 0
    local holdingReload = ply:KeyDown(IN_RELOAD)
    local hasGasCan = ply:GetAmmoCount(self.GasCanAmmoType) > 0

    -- Hold Reload to refuel: only while the saw is OFF, empty, and the
    -- player actually has a gas can. Otherwise fall through to the normal
    -- tap-to-toggle logic below (so an empty saw with no gas can still
    -- gives an instant "empty" click instead of forcing a pointless hold).
    if not isOn and isEmpty and holdingReload and hasGasCan and not self.isHoldAttacking then
        if not self.isRefueling then
            self.isRefueling = true
            self.refuelStartTime = CurTime()
        end

        if CurTime() >= self.refuelStartTime + self.RefuelDuration then
            self.isRefueling = false
            self.refuelStartTime = nil

            if ply:GetAmmoCount(self.GasCanAmmoType) > 0 then
                if SERVER then
                    ply:RemoveAmmo(1, self.GasCanAmmoType)
                    self:SetNWInt("Gasoline", self.MaxGasoline)
                end
                self:EmitSound("items/ammo_pickup.wav", 75, 100, 1, CHAN_ITEM)
            else
                -- Ammo vanished mid-hold (e.g. dropped/transferred) - fail safely
                self:EmitSound("Weapon_Pistol.Empty")
            end
        end

        return
    elseif self.isRefueling then
        -- Player let go of Reload (or the state changed) before finishing
        self.isRefueling = false
        self.refuelStartTime = nil
    end
    ...
    if ply:KeyPressed(IN_RELOAD) then
        ...
        if newState then
            ...
            -- Turn-on sound sequence (server-only, one-shot)
            if SERVER then
            ...
            -- Turn-off sound and stop idle loop (server-only, one-shot)
            if SERVER then
...
function SWEP:SecondaryAttack()
    ...
    if not self:GetNWBool("chainsawon", false) then
        if self.BaseClass and self.BaseClass.SecondaryAttack then
...
function SWEP:Think()
    ...
    -- If the chainsaw is dropped and running, damage players who get too close
    if SERVER and is_on and not IsValid(self:GetOwner()) then
...
function SWEP:CheckDangerTouch()
    ...
    if IsValid(self:GetOwner()) then return end -- Skip check if it has an owner

    -- Check every 0.5 seconds to avoid overloading the server
    local nextCheck = self.nextDangerCheck or 0
    if CurTime() < nextCheck then return end
    self.nextDangerCheck = CurTime() + 0.5

    local pos = self:GetPos()
    -- Find all players within 45 units
